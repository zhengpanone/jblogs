...
class SEG(object):
    def __init__(self):
        ...
        with open(os.path.join(curpath, "main.dic"), encoding="utf-8") as in_file:
            self.set([x.rstrip() for x in in_file])
        with open(os.path.join(curpath, "suffix.dic"), encoding="utf-8") as in_file:

            # [x.rstrip().decode('utf-8') for x in in_file]
